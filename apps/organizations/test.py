...
class OrganizationTests(APITestCase):
    ...
    def test_register_device(self):
        response = self.client.post('/api/organizations/register_device/', self.device_data)
    ...
    def test_create_playlist(self):
        ...
        response = self.client.post('/api/organizations/create_playlist/', playlist_data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Playlist.objects.count(), 1)

    def test_sync_device(self):
        Device.objects.create(name="Test Device", serial_number="ABC123", exit_password="securepass", owner=self.user)
